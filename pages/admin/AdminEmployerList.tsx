...
import { getAllEmployers, verifyEmployerProfile } from '../../services/employerService';
...
const AdminEmployerList = () => {
  ...
  useEffect(() => {
    ...
    const fetchEmployers = async () => {
      ...
      // Sort by status (unverified first) then by name
      const sorted = [...data].sort((a, b) => {
        if (a.isVerified === b.isVerified) return a.organizationName.localeCompare(b.organizationName);
  ...
  const handleToggleVerify = async (uid: string, currentStatus: boolean) => {
    if (!confirm(`Are you sure you want to ${currentStatus ? 'unverify' : 'verify'} this employer?`)) return;
    
    const result = await verifyEmployerProfile(uid, !currentStatus);
    if (result.success) {
      setEmployers(prev => prev.map(emp => 
        emp.uid === uid ? { ...emp, isVerified: !currentStatus } : emp
      ));
    } else {
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredEmployers.map((emp, idx) => (
                    <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="p-4">
                        <button 
                          onClick={() => handleToggleVerify(emp.uid, emp.isVerified)}
                        ...
                        >
                          {emp.isVerified ? 'Verified' : 'Verify Now'}
                        </button>
                      </td>
                      <td className="p-4">
