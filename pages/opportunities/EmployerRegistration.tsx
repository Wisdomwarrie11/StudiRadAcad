...
import { registerEmployer, verifyOTP, resendOTP } from '../../services/employerService';
...
const EmployerRegistration = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // If email failed to send, notify the user but stay on the OTP step
        if (!result.emailSent) {
          setResendStatus(`Account created, but we couldn't send the code: ${result.emailError || 'Unknown error'}. Please click Resend Code.`);
        }
      } else {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    const result = await verifyOTP(formData.email, otp, registeredUid);
    if (result.success) {
  ...
  const handleResendOTP = async () => {
    setResending(true);
    setResendStatus('');
    const result = await resendOTP(formData.email, formData.organizationName);
    if (result.success) {
      setResendStatus('New code sent! Please check your email.');
    } else {
