  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setStep(3); // Show the "Check Email" step
      } else {
                  ...
                  {step === 3 && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                       <div className="bg-emerald-50 p-6 rounded-[2.5rem] border border-emerald-100 text-center">
                          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm border border-emerald-100 text-emerald-500">
                             <Mail size={32} />
                          </div>
                       ...
                       <Link 
                        to="/employer/login"
                        className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black flex items-center justify-center gap-2 hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/10"
                       >
                         Go to Login <ArrowRight size={20} />
                       </Link>

                       <p className="text-center text-xs text-slate-400 font-bold">
                         Did you click the link? <button type="button" onClick={() => window.location.reload()} className="text-brand-primary hover:underline">Refresh Page</button>
                       </p>
                    </div>
                  )}
            </form>
