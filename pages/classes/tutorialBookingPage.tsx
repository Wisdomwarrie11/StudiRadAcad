...
const PAYSTACK_PUBLIC_KEY = import.meta.env.VITE_PAYSTACK_PUBLIC_KEY || "pk_live_a35b5eef4a79e10f6f06b9f1a7a56a7424ccfbc6";
...
const TutoringBookingPage: React.FC = () => {
  ...
  const handlePaymentSuccess = async (response: any) => {
    ...
    // Construct requested WhatsApp message format
    let waMessage = "";
    if (bookingMode === 'subscription') {
      ...
      const totalHours = cart.reduce((acc, item) => acc + (item.hours * item.days), 0);
    ...
    const redirectToWhatsApp = () => {
      window.open(`https://wa.me/2347041197027?text=${encodeURIComponent(waMessage)}`, "_blank");
      navigate('/classes');
    };

    try {
      // Save data to Firestore (Modular Syntax)
      ...
      console.error("Payment successful but database error occurred:", err);
      // Fail-safe: Always open WhatsApp even if database save fails so admin can see the ref
      redirectToWhatsApp();
    } finally {
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20">
      ...
      <div className="bg-brand-dark text-white py-3 px-4 sticky top-0 z-[60] shadow-xl border-b border-white/5">
        <div className="container mx-auto flex flex-col md:flex-row justify-center items-center gap-4">
          <div className="flex items-center gap-2 text-amber-400 font-black">
            <span className="animate-bounce"><FaGift /></span>
      ...
      <div className="container mx-auto px-4 max-w-6xl pt-10">
        <button onClick={() => navigate('/classes')} className="mb-10 flex items-center text-slate-500 hover:text-indigo-600 font-black transition-all">
          <span className="mr-2"><FaArrowLeft /></span> Back to Academy
        </button>
              ...
              {step === 1 ? (
                ...
                >
                  <div className="mb-10">
                    <h3 className="text-2xl font-black text-slate-900 flex items-center gap-3 mb-2">
                       <span className="text-indigo-600"><FaUserGraduate /></span> Student Profile
                    </h3>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-12">
                    <div className="flex bg-white p-2 rounded-3xl shadow-sm border border-slate-200 max-w-xl mx-auto ring-1 ring-slate-100">
                        ...
                        >
                            <span className={bookingMode === 'instant' ? 'text-amber-400' : ''}><FaBolt /></span> Instant Help
                        </button>
                        <button 
                        ...
                        >
                            <span className={bookingMode === 'subscription' ? 'text-indigo-400' : ''}><FaLayerGroup /></span> Monthly Plans
                        </button>
                    </div>
                    ...
                    {bookingMode === 'subscription' ? (
                            ...
                            {SUBSCRIPTION_PACKAGES.map(pkg => (
                                ...
                                <ul className="space-y-4 mb-8 flex-grow">
                                    ...
                                    <li key={i} className="flex items-start gap-3 text-[11px] font-bold text-slate-600 leading-tight">
                                        <span className="text-emerald-500 mt-0.5 shrink-0"><FaCheckCircle /></span>
                                        <span>{f}</span>
                                    </li>
                                    ))}
                                </ul>
                                    ...
                                    <div className="mt-8 flex flex-wrap gap-2">
                                        {subCourses.map(course => (
                                            <span key={course} className="bg-indigo-600 text-white px-4 py-2 rounded-full text-xs font-bold flex items-center gap-2">
                                                {course} <span className="cursor-pointer" onClick={() => setSubCourses(subCourses.filter(c => c !== course))}><FaTrash /></span>
                                            </span>
                                        ))}
                                    </div>
                                </motion.div>
                        ...
                        <div className="bg-white rounded-[3rem] p-10 shadow-sm border border-slate-100">
                            <div className="mb-10">
                            <h3 className="text-2xl font-black text-slate-900 flex items-center gap-3 mb-2">
                                <span className="text-amber-500"><FaBolt /></span> Instant Clarity (₦3,000 / hr)
                            </h3>
                            ...
                            <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100">
                                <div className="flex items-center gap-3 mb-4">
                                    <span className="text-amber-500"><FaSearchPlus /></span>
                    ...
                    <div className="flex items-center justify-between mb-10">
                        <h3 className="text-2xl font-black text-slate-900 flex items-center gap-4">
                            <span className="text-indigo-600 shrink-0"><FaCalendarAlt /></span> <span>Start Date</span>
                        </h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        ...
                        <span className="text-indigo-600 mt-1 shrink-0">
                    ...
                    <div className="lg:hidden mt-8 space-y-6">
                        <div className="bg-slate-900 text-white rounded-[3rem] p-8 shadow-xl border border-white/5 overflow-hidden relative">
                            ...
                            >
                                {isProcessing ? (
                                    <><span className="animate-spin"><FaPlus /></span> Processing...</>
                                ) : (
                            ...
                            <div className="mt-6 pt-6 border-t border-white/5 flex gap-4 items-center">
                                <span className="text-emerald-400 shrink-0"><FaShieldAlt /></span>
          ...
          {step === 2 && (
            <aside className="lg:w-[420px] hidden lg:block">
                <div className="bg-slate-900 text-white rounded-[3.5rem] p-10 shadow-2xl sticky top-28 border border-white/5 overflow-hidden">
                    ...
                    >
                        {isProcessing ? <><span className="animate-spin"><FaPlus /></span> Processing...</> : <><span className="text-slate-950"><FaCreditCard /></span> Pay with Paystack</>}
                    </button>
                    
                    <div className="mt-8 bg-white/5 rounded-3xl p-6 flex gap-5 items-start border border-white/5">
                        <span className="text-emerald-400 mt-1"><FaShieldAlt /></span>
                        <div>
