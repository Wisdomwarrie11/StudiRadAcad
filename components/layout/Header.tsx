...
const Header: React.FC = () => {
  ...
  const headerBgClass = isScrolled || location.pathname !== '/' 
    ...
    : 'bg-brand-dark/90 backdrop-blur-sm py-5';
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex justify-between items-center text-white">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <div className="relative group">
              <button className="flex items-center gap-1 text-sm font-medium hover:text-brand-accent transition-colors">
                Academy <ChevronDown size={14} />
              </button>
              <div className="absolute top-full left-0 w-56 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                <div className="bg-white rounded-xl shadow-xl overflow-hidden py-2 text-gray-800">
                  <Link to="/classes" className="block px-4 py-2 hover:bg-gray-50 hover:text-brand-primary">Live Classes</Link>
                  <Link to="/courses" className="block px-4 py-2 hover:bg-gray-50 hover:text-brand-primary">Courses</Link>
                  {/* <Link to="/tutoring-booking" className="block px-4 py-2 hover:bg-gray-50 hover:text-brand-primary">1-on-1 Tutoring</Link> */}
                </div>
              </div>
            </div>
        ...
        <div className="md:hidden absolute top-full left-0 w-full bg-brand-dark border-t border-gray-700 shadow-xl min-h-screen">
          <div className="flex flex-col p-6 gap-4">
            ...
            <div className="space-y-2">
              <button onClick={() => setResourcesOpen(!resourcesOpen)} className="flex items-center justify-between w-full text-white text-lg font-medium">
            ...
            <div className="space-y-2">
              <button onClick={() => setAcademyOpen(!academyOpen)} className="flex items-center justify-between w-full text-white text-lg font-medium">
                Academy <ChevronDown size={16} className={`transition-transform ${academyOpen ? 'rotate-180' : ''}`} />
              </button>
              {academyOpen && (
                <div className="pl-4 space-y-2 border-l border-gray-700 ml-2">
                  <Link to="/classes" onClick={() => setMobileMenuOpen(false)} className="block text-gray-300">Live Classes</Link>
                  <Link to="/courses" onClick={() => setMobileMenuOpen(false)} className="block text-gray-300">Courses</Link>
                  {/* <Link to="/tutoring-booking" onClick={() => setMobileMenuOpen(false)} className="block text-gray-300">Tutorials</Link> */}
                </div>
              )}
            </div>
